...
class ScannedStringStage(Stage):
    def run(self, stage_input):
        return stage_input.replace("$", "S").replace("1", "I").replace("0", "O").strip("<").strip()


class FilterValidWordsOnlyStage(Stage):
    def run(self, stage_input: List[str]):
        return list(filter(lambda word: re.match(VALID_CHARACTERS_ONLY, word), stage_input))
...
class StringFieldMatchStage(Stage):
    def __init__(self, field_definition: List[str], definition_threshold: int):
        ...
        self.definition_checker = ProximityChecker(connect_words(self.field_definition), definition_threshold)
